...
<body>
	<div class="container">
		...
		<h3>
			所有用户 <a href="/admin/users/add" type="button"
		...
		<c:if test="${empty userList}">
			<div class="alert alert-warning" role="alert">
				<span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>User表为空，请<a
					href="/admin/users/add" type="button"
					class="btn btn-primary btn-sm">添加</a>
			</div>
		</c:if>
			...
			<table class="table table-bordered table-striped">
				...
				<c:forEach items="${userList}" var="user">
					<tr>
						...
						<td>${user.password}</td>
						<td><a href="${ctx}admin/users/show/${user.id}" type="button"
							class="btn btn-sm btn-success">详情</a> <a
							href="/admin/users/update/${user.id}" type="button"
							class="btn btn-sm btn-warning">修改</a> <a
							href="/admin/users/delete/${user.id}" type="button"
