...
/**
 * @author yuzhao.yang
 * @description:尝试不登录去获取信息
 * @time:2017年7月10日 下午4:37:30
 */
public class SingleBrandProcessByJsoupWithoutLogin {
	...
	public static void process(String name) throws Exception {
		...
		Map<String, String> afterCookies = new HashMap<String, String>();
		for (String key : cookies.keySet()) {
			afterCookies.put(key, cookies.get(key));
		}
		...
		logger.info(Thread.currentThread().getName() + "---下载" + name
				+ "第一页文档!");
		response = SpiderJsoupUtil.getContent(url, data, afterCookies,
				Method.GET, null);

		// 这是第一页面的文档
		Document document = response.parse();

		processAfter(url, afterCookies, data, document);
	...
	public static void processDetailDoc(Document doc, BrandDetail detail) {
		...
		for (Element meta : metas) {
			if (meta.outerHtml().contains("申请人地址")) {
				String content = meta.attr("content");
				String address = content.substring(content.indexOf("地址:") + 3);
				address = address.substring(0, address.length() - 1);
				detail.setApplicant_address_en(address);
			}
		}
