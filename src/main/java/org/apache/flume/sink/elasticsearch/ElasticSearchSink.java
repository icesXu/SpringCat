...
public class ElasticSearchSink extends AbstractSink implements Configurable {
	...
	public static void main(String[] args) throws EventDeliveryException {
		ElasticSearchSink sink = new ElasticSearchSink(false);
		sink.process();
	}
		...
		try {
			...
			for (count = 0; count < batchSize; ++count) {
				...
				String realIndexType = BucketPath.escapeString(indexType,
						event.getHeaders());
				logger.error("realIndexType：" + realIndexType);
	...
	public void configure(Context context) {
		if (!isLocal) {
			...
			Preconditions
					.checkState(serverAddresses != null
							&& serverAddresses.length > 0, "Missing Param:"
							+ HOSTNAMES);
		}
		...
		elasticSearchClientContext.putAll(context
				.getSubProperties(CLIENT_PREFIX));
		...
		serializerContext.putAll(context
				.getSubProperties(INDEX_NAME_BUILDER_PREFIX));
	...
	private long parseTTL(String ttl) {
		...
		while (matcher.find()) {
			if (matcher.group(2).equals("ms")) {
				...
				return TimeUnit.SECONDS.toMillis(Integer.parseInt(matcher
						.group(1)));
			} else if (matcher.group(2).equals("m")) {
				return TimeUnit.MINUTES.toMillis(Integer.parseInt(matcher
						.group(1)));
			} else if (matcher.group(2).equals("h")) {
				return TimeUnit.HOURS.toMillis(Integer.parseInt(matcher
						.group(1)));
			} else if (matcher.group(2).equals("d")) {
				return TimeUnit.DAYS
						.toMillis(Integer.parseInt(matcher.group(1)));
			} else if (matcher.group(2).equals("w")) {
				return TimeUnit.DAYS.toMillis(7 * Integer.parseInt(matcher
						.group(1)));
